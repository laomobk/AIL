...
def _visit_param_list(
    ...
    for param in param_list.arg_list:
        ...
        symbol_table.mangle(symbol)
        param.expr.value = symbol.name
    ...
    def _visit_try_stmt(self, stmt: ast.TryCatchStmtAST):
        ...
        for case in stmt.catch_cases:
            ...
            if case.exc_expr and case.alias:
                ...
                case.alias_expr.symbol = self._analyze_and_fill_symbol(
                    Symbol(case.alias), CTX_STORE, ignore_nonlocal=True
                )
